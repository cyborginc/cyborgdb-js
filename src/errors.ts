 ...
 */
import { ErrorResponseModel, HTTPValidationError } from './models';
...
const DEBUG = typeof process !== 'undefined'
  && (process.env?.CYBORGDB_DEBUG === '1' || process.env?.CYBORGDB_DEBUG === 'true');
...
export function handleApiError(error: unknown): never {
  ...
  if (hasResponse(error)) {
    debugLog("HTTP Status Code:", error.response.statusCode || error.response.status);
    debugLog("Response Headers:", JSON.stringify(error.response.headers, null, 2));
    debugLog("Response Body:", hasBody(error) ? error.body : error.response.body || error.response.data);
