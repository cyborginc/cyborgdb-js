...
function captureApi() {
	...
	const api: any = {
		getIndexInfoV1IndexesDescribePost: rec("describe", {
			indexName: "idx",
			isTrained: false,
			dimension: 4,
			metric: "euclidean",
		...
		upsertVectorsBinaryV1VectorsUpsertBinaryPost: rec("upsertBinary", {
			status: "success",
		}),
		queryVectorsBinaryV1VectorsQueryBinaryPost: rec("queryBinary", {
			results: [],
		}),
	};
	return { api, calls };
...
describe.skip("CyborgDB KMS — offline contract (no service)", () => {
	...
	it("createIndex rejects when neither indexKey nor kmsName is provided", async () => {
		await expect(
			offlineClient().createIndex({ indexName: "idx" }),
	...
	it("CreateIndexRequest with only kmsName omits index_key on the wire", () => {
		const wire = JSON.parse(
			JSON.stringify(
				CreateIndexRequestToJSON({ indexName: "idx", kmsName: "slot" }),
			),
		);
...
describeIfConfigured(
	"CyborgDB KMS — mode 2 (fully KMS-managed via aws / Secrets Manager)",
	() => {
		if (!KMS_SM) {
...
describeIfConfigured(
	"CyborgDB KMS — mode 1 (SDK-supplied KEK, no kmsName)",
	() => {
...
describeIfConfigured("CyborgDB KMS — negative paths (config-agnostic)", () => {
	...
	it("rejects createIndex when neither kmsName nor indexKey is supplied (server-side)", async () => {
		...
		const res = await fetch(`${BASE_URL}/v1/indexes/create`, {
			...
			body: JSON.stringify({
				index_name: `neg_empty_${Date.now().toString(36)}`,
				dimension,
			}),
		});
...
describeIfConfigured(
	"CyborgDB KMS — negative paths (real-provider contract)",
	() => {
		if (!KMS_REAL) {
			it.skip("skipped: CYBORGDB_KMS_NAME_REAL not set", () => undefined);
			return;
		...
		it("rejects loadIndex with an extraneous indexKey on a real-provider index", async () => {
			...
			try {
				const extraneous = Client.generateKey();
				await expect(
					client.loadIndex({ indexName, indexKey: extraneous }),
				).rejects.toThrow();
			} finally {
				try {
...
describeIfConfigured(
	"CyborgDB KMS — negative paths (SDK-supplied KEK contract)",
	() => {
		...
		it("fails to query after loadIndex with wrong indexKey on an SDK-supplied-KEK index", async () => {
			...
			try {
				...
				await expect(
					(async () => {
						const reloaded = await client.loadIndex({
							indexName,
							indexKey: wrongKey,
						});
...
describeIfConfigured("CyborgDB KMS — mixed-mode concurrency", () => {
	if (!KMS_REAL || !KMS_SM) {
		it.skip("skipped: requires both CYBORGDB_KMS_NAME_REAL and CYBORGDB_KMS_NAME_SM", () =>
			undefined);
		return;
	}
	...
	it("creates indexes against aws-kms, aws/SM, and SDK-supplied paths in parallel", async () => {
		...
		expect(listed).toEqual(
			expect.arrayContaining([kmsKmsName, kmsSmName, sdkName]),
		);
	...
	it("loads pre-existing indexes across all three paths in parallel", async () => {
		...
		try {
			...
			await Promise.all(
				created.map((idx) => idx.deleteIndex().catch(() => undefined)),
			);
		}
	});
});
