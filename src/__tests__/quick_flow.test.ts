...
function checkQueryResults(
	...
	const resultIds: number[][] = results.map((queryResults) =>
		queryResults.map((res) => parseInt(res.id, 10)),
	);

	if (
		neighbors.length !== resultIds.length ||
		neighbors[0].length !== resultIds[0].length
	) {
		throw new Error(
...
function checkMetadataResults(
	...
	const resultIds: number[][][] = normalizedResults.map((result) =>
		result.map((queryResults) => queryResults.map((res) => safeInt(res.id))),
	);
	...
	for (let idx = 0; idx < resultIds.length; idx++) {
		...
		for (let i = 0; i < numQueries; i++) {
			...
			const groundtruthIds: number[] = groundtruthIndices
				.filter(
					(idx: number) => idx !== -1 && idx >= 0 && idx < candidates.length,
				)
...
describe("TestUnitFlow", () => {
	...
	test("test_01_untrained_upsert", async () => {
		...
		const expectedIds = Array.from({ length: numUntrainedVectors }, (_, i) =>
			String(i),
		);
	...
	test("test_02_untrained_query_no_metadata", async () => {
		...
		console.log(
			`Untrained Query (No Metadata). Expected recall: ${untrainedRecall}, got ${recall}`,
		);
	...
	test("test_04_untrained_get", async () => {
		...
		for (let i = 0; i < getResults.length; i++) {
			...
			const metadataStr = getResult.metadata
				? JSON.stringify(
						getResult.metadata,
						Object.keys(getResult.metadata).sort(),
					)
				: "null";
	...
	test("test_06_upsert_to_trigger_auto_train", async () => {
		...
		const expectedIds = Array.from({ length: autoTrainTrigger }, (_, i) =>
			String(i),
		);
		expect(results.ids.sort()).toEqual(expectedIds.sort());
	});
	...
	test("test_07_wait_for_auto_train", async () => {
		...
		for (let attempt = 0; attempt < numRetries; attempt++) {
			...
			if (trained) {
				...
				console.log(
					`Index not trained yet, retrying... (${attempt + 1}/${numRetries})`,
				);
			}
		}
	...
	test("test_08_upsert_remaining_vectors", async () => {
		...
		const results = await index.listIds();
		console.log(`Total IDs in index: ${results.ids.length}`);
		const expectedIds = Array.from({ length: totalNumVectors }, (_, i) =>
			String(i),
		);
		expect(results.ids.sort()).toEqual(expectedIds.sort());
	});
	...
	test("test_09_retrain_with_n_lists", async () => {
		...
		for (let attempt = 0; attempt < numRetries; attempt++) {
			...
			if (!isCurrentlyTraining) {
				...
				console.log(
					`Index still training, retrying... (${attempt + 1}/${numRetries})`,
				);
			}
		}
	...
	test("test_11_trained_query_no_metadata", async () => {
		...
		const recall = checkQueryResults(results, trainedNeighbors, numQueries);
		console.log(
			`Trained Query (No Metadata). Expected recall: ${trainedRecall}, got ${recall}`,
		);
	...
	test("test_12_trained_query_metadata", async () => {
		...
		const expectedThresholds = baseThresholds.map(
		...
		for (let idx = 0; idx < recalls.length; idx++) {
			...
			if (idx < 17) {
				...
				console.log(`Metadata Query #${idx + 1}`);
				console.log(
					`Metadata filters: ${JSON.stringify(metadataQueries[idx])}`,
				);
				console.log(
	...
	test("test_14_delete", async () => {
		...
		const idsToDelete = Array.from({ length: numUntrainedVectors }, (_, i) =>
			String(i),
		);
