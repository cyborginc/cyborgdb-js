...
describe("getDemoApiKey", () => {
	...
	test("should successfully generate demo API key", async () => {
		...
		expect([
			...
			"https://staging-api.cyborgdb.co/v1/api-key/manage/create-demo-key",
			"https://dev-api.cyborgdb.co/v1/api-key/manage/create-demo-key",
		]).toContain(callArgs[0]);
	...
	test("should generate demo API key with custom description", async () => {
		...
		expect(callArgs[1].body).toBe(
			JSON.stringify({ description: customDescription }),
		);
	});
	...
	test("should throw error when apiKey is missing in response", async () => {
		...
		await expect(getDemoApiKey()).rejects.toThrow(
			"Demo API key not found in response",
		);
	...
	test("should throw error on HTTP error status", async () => {
		...
		// Call the function and expect error
		await expect(getDemoApiKey()).rejects.toThrow(
			"Failed to generate demo API key",
		);
	...
	test("should throw error on network error", async () => {
		...
		// Call the function and expect error
		await expect(getDemoApiKey()).rejects.toThrow(
			"Failed to generate demo API key: Network connection failed",
		);
		expect(consoleErrorSpy).toHaveBeenCalled();
	});
	...
	test("should throw error on timeout", async () => {
		...
		// Call the function and expect error
		await expect(getDemoApiKey()).rejects.toThrow(
			"Failed to generate demo API key: Request timeout",
		);
		expect(consoleErrorSpy).toHaveBeenCalled();
	});
	...
	test("should handle non-Error exceptions", async () => {
		...
		// Call the function and expect error
		await expect(getDemoApiKey()).rejects.toThrow(
			"Failed to generate demo API key: String error",
		);
	});
});
