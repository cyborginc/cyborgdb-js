...
describe("CyborgDB API Contract Tests", () => {
	...
	describe("07 - Client.createIndex()", () => {
		it("should create DiskIVF index with explicit dimension and custom metric", async () => {
			const tempIndexName = `temp_diskivf_${Date.now().toString(36)}`;
		...
		it("should create DiskIVF index without dimension (auto-detected on upsert)", async () => {
			const tempIndexName = `temp_diskivf_auto_${Date.now().toString(36)}`;
			const tempIndexKey = Client.generateKey();
	...
	describe("08 - EncryptedIndex Properties", () => {
		it("should expose index name via getIndexName()", async () => {
			...
			if (!testIndex) {
				throw new Error(
					"testIndex is null or undefined - it was not created properly in section 07",
				);
			}
	...
	describe("11 - EncryptedIndex.upsert()", () => {
		...
		it("should reject when neither items nor ids/vectors provided", async () => {
			await expect(testIndex.upsert({})).rejects.toThrow();
		});
	});
	...
	describe("15 - EncryptedIndex.query() patterns", () => {
		it("should query with multiple test patterns", async () => {
			...
			const response1 = await testIndex.query({
				queryVectors: singleVector,
				topK: 3,
			});
